...
async def seed_database(conn: asyncpg.Connection):
    """Seed initial curriculum, lessons, and assessment data if not already present."""
    count = await conn.fetchval("SELECT COUNT(*) FROM curriculum WHERE user_id IS NULL")
    if count and count > 0:
        print("[INFO] Curriculum table already has base records. Skipping initial seeding.")
        return

    print("[INFO] Seeding initial curriculum, lessons, and assessments into database...")
    ...
    for c in curriculums:
        curr_id = await conn.fetchval("""
            INSERT INTO curriculum (difficulty_level, category, sequence_order, target_language, native_language)
            VALUES ($1, $2, $3, 'en', 'en')
    ...
    # 3. Insert Base Assessments for en, hi, kn, uz
    assessments_data = [
        # ENGLISH
        {
    ...
    print("[INFO] Database curriculum and assessments seeded successfully!")
...
async def main():
    ...
    clean_dsn = urllib.parse.urlunparse((parsed.scheme, parsed.netloc, parsed.path, '', '', ''))
    
    ssl_ctx = None
    if parsed.hostname and parsed.hostname not in ["localhost", "127.0.0.1", "db"]:
        ssl_ctx = ssl.create_default_context()
        ssl_ctx.check_hostname = False
        ssl_ctx.verify_mode = ssl.CERT_NONE

    print(f"Connecting to seed the database at {parsed.hostname}...")
    try:
